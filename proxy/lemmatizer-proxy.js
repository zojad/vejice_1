...
server.on("error", (err) => {
  if (err && err.code === "EADDRINUSE") {
    probeExistingProxyHealth()
      .then((healthy) => {
        if (healthy) {
          console.log(`[lemmatizer-proxy] Port ${PORT} already in use. Reusing healthy existing proxy.`);
          process.exit(0);
          return;
        }
        console.error(
          `[lemmatizer-proxy] Port ${PORT} already in use, but the existing proxy does not answer /health. Stop the stale process and restart.`
        );
        process.exit(1);
      })
      .catch(() => {
        console.error(
          `[lemmatizer-proxy] Port ${PORT} already in use, but the existing proxy health could not be verified. Stop the stale process and restart.`
        );
        process.exit(1);
      });
    return;
